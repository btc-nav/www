...
import { Hidden, Link, Drawer, Divider } from '@material-ui/core';
...
class NavBar extends PureComponent {
  ...
  onScroll = () => {
    ...
    if (navHeight > window.innerHeight - 234) {
      if (scrollTop > document.body.clientHeight / 2) {
        top =
          -(scrollTop / (document.body.clientHeight / 2) - 1) *
          (navHeight - window.innerHeight + 234 + 166);
      } else {
        top = 0;
      }
    }
  ...
  renderWebNavList = () => {
      ...
      return (
        ...
        >
          {skeletonList.map((key, index) => (
            <Box pt={1} pr={2} pb={1} pl={2} key={`${key}-${index}`}>
    ...
    return (
      <Fragment>
          ...
          {(tagList || []).map(({ tag, tag_en }, index) => {
            ...
            return (
              <Link
                ...
                className={classnames(
                  styles.link,
                  activeKey === encodeURI(tag_en) ? styles.link_active : {}
                )}
                color='textPrimary'
                underline='none'
              >
        ...
        {sticky ? <Box style={{ width: 273.19 }} /> : null}
      </Fragment>
    );
  };
  ...
  render() {
    ...
    return (
      <>
        <Hidden lgUp>
          <Drawer open={drawerVisible} onClose={onClose}>
            <Box
              display='flex'
              flexDirection='column'
              style={{
            ...
            >
              ...
              <Divider />
              <Box flex='1' style={{ overflow: 'auto' }} onClick={onClose}>
                {(tagList || []).map(({ tag, tag_en }, index) => {
                  let tagName = language === 'zh' ? tag : tag_en;
                  return (
                    <Link
        ...
        <Hidden mdDown>{this.renderWebNavList()}</Hidden>
      </>
    );
  }
}
